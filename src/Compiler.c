...
void CompileProgramTree(char** outCode, size_t* outLength)
{
    ...
    StreamWrite(stream, watermark, strlen(watermark));
    ...
    for (int i = 0; i < programNodes.length; ++i)
    {
        ...
        if (codeLength != 0)
        {
            const char* comment = "// Module: ";
            const char* newline = "\n";
            StreamWrite(stream, comment, strlen(comment));
            StreamWrite(stream, node->path, strlen(node->path));
            StreamWrite(stream, newline, strlen(newline));
        }
