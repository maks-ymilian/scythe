...
static void ProcessAssignment(NodePtr assignment)
{
	...
	{
		NodePtr* node = GetReference(assignment.ptr);
		if (node->type == Node_ExpressionStatement)
			((ExpressionStmt*)node->ptr)->expr = GetAssignmentRight(assignment);
		else if (node->type == Node_VariableDeclaration)
			*node = AllocASTNode(
				&(ExpressionStmt){
					.expr = GetAssignmentRight(assignment),
				},
...
static void VisitStatement(NodePtr* node, bool endOfFunction)
{
	...
	case Node_Input:
	{
		...
		AddReference(varDecl, &input->varDecl);
		break;
	}
